...
**理由:** テスト用mock、D-Bus/CLI選択、安全性検査の再利用を容易にする。当初想定したtimer/socket/pathとcronへの拡張方針はADR-023で見直した。

## ADR-006: 言語は英語フォールバックとする

**決定:** 日本語localeは日本語、それ以外は英語。文字列を初期版から分離する。手動言語選択を予約する当初方針はADR-023で見直した。
...
## ADR-020: drop-in管理を専用の安全なファイル境界として実装する

**決定:** drop-inはcanonical unit IDに対応する`<unit>.d`直下の`.conf`だけを扱い、service本体と同等の所有者・permission・symlink・リビジョン検査を適用する。保存前に隔離したserviceとdrop-inを`systemd-analyze`で検証する。

**理由:** `systemctl edit`の外部エディター起動をUIから制御するより、既存の原子的保存・競合拒否・検証境界を再利用する方が変更対象を予測できるため。

## ADR-021: service削除は停止・無効化と耐久化済みバックアップを必須にする

**決定:** 発見起点に直接配置されたserviceだけを削除対象とし、activeまたはenabledなserviceは拒否する。本体と管理対象drop-inを非公開バックアップへ保存・fsyncし、直前再検査後に削除する。

**理由:** 実行中・自動起動対象の定義を消す誤操作と、復元不能な削除を同時に防ぐため。

## ADR-022: v0.3の復元は最新バックアップ・非上書きに限定する

**決定:** UIからの復元は最新の有効なバックアップだけを対象とし、serviceまたはdrop-inディレクトリが存在する場合は拒否する。履歴選択と保持期限管理は後続へ延期する。

**理由:** 版選択やマージを導入せず、missing状態から元の一式を戻す単純で安全な経路を先に提供するため。

## ADR-023: 現在の製品範囲をsystemdユーザーserviceへ限定する

**決定:** `.timer`、`.socket`、`.path`、関連ユニット表示、ユニット種別別UI、system service、高度ログ、追加Debian系環境の検証は、必要性が具体化するまで版や期限を持たない将来構想とする。cronバックエンドとアプリ内の手動言語選択は実装計画に含めない。

**理由:** 現在想定する個人規模のユーザーservice管理では、ユニット種別や関係表示を拡大する複雑さに見合う需要がない。複雑なログ解析には既存の専門手段があり、利用環境もUbuntuを中心とするDebian由来環境に限られる。能力モデルとポート分離は既存の安全性とテスト容易性のため維持し、将来の範囲拡大を約束するものとは扱わない。
